...
@Resource(name = RestConstants.VERSION_1 + AppointmentRestController.APPOINTMENT_SCHEDULING_REST_NAMESPACE + "/monthlyreporting",
        supportedClass = Appointment.class, supportedOpenmrsVersions = {"1.9.*", "1.10.*", "1.11.*", "1.12.*", "2.0.*", "2.1.*", "2.2.*"})
public class MonthlyAppointmentsResource1_9 extends DataDelegatingCrudResource<Appointment> {
    ...
    public Integer getDairlyCount(Appointment appointment) {
        return Context.getService(AppointmentService.class).getAppointmentDailyCount(appointment);
    }
