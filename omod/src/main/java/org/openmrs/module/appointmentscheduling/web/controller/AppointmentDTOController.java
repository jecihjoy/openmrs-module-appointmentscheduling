...
public class AppointmentDTOController {
    ...
    public AppointmentDTO saveAppointment(@RequestBody AppointmentDTO appointmentDTO) throws ParseException {
        ...
        TimeSlot timeSlot = service.getTimeslotForAppointment(Context.getLocationService().getLocationByUuid(appointmentDTO.getLocation()),
                Context.getProviderService().getProviderByUuid(appointmentDTO.getProvider()), appointment.getAppointmentType(), appointmentDate);
        if (timeSlot == null) {
            timeSlot = service.createTimeSlotUsindProviderSchedule(appointmentDate, Context.getProviderService().getProviderByUuid(appointmentDTO.getProvider()),
                    Context.getLocationService().getLocationByUuid(appointmentDTO.getLocation()));
        }
