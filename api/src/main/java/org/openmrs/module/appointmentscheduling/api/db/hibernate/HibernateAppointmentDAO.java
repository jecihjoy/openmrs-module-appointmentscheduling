...
public class HibernateAppointmentDAO extends HibernateSingleClassDAO
    ...
    @Override
    public List<Appointment> getDailyAppointments(Date date, Provider provider, AppointmentType appointmentType, AppointmentStatus status,
                                                  Patient patient, VisitType visitType, Location location) throws DAOException {
        ...
        if (status != null && date != null) {
            switch (status) {
                //daily schedules
                case SCHEDULED:
                    stringQuery = "SELECT appointment FROM Appointment AS appointment WHERE appointment.voided = 0";
                    break;

                //defaulters of a certain day
                case MISSED:
                    stringQuery = "SELECT appointment FROM Appointment AS appointment WHERE appointment.visit is null" +
                            "   AND DATE_FORMAT(appointment.timeSlot.startDate,'%Y-%m-%d') < CURDATE()";
                    break;

                //attended
                case COMPLETED:
                    stringQuery = "SELECT appointment FROM Appointment AS appointment WHERE appointment.visit is not null";
                    break;
            }
...
/*            if (visitType != null)
            ...
            if (location != null)
                stringQuery += " AND appointment.timeSlot.appointmentBlock.location = :location";
            if (date != null)
                stringQuery += " AND DATE_FORMAT(appointment.timeSlot.startDate,'%Y-%m-%d') = :dateSelected ";
            if (patient != null) {
                stringQuery += " AND appointment.patient=:patient";
            }
            ...
            query.setParameter("dateSelected", getStringDate(date));
...
/*        if (visitType != null)
    ...
    @Override
    public List<Appointment> getMonthlyAppointmentsReports(Date fromDate, Date toDate, Provider provider, AppointmentType appointmentType,
                                                           AppointmentStatus status, Patient patient, VisitType visitType, Location location) throws DAOException {
        ...
        if (status != null && fromDate != null && toDate != null) {
            ...
            stringQuery += "   AND DATE_FORMAT(appointment.timeSlot.startDate,'%Y-%m-%d') BETWEEN :fromDate AND :toDate"  +
                            "   AND appointment.timeSlot.startDate IN (" +
                    "  SELECT DISTINCT  appointment.timeSlot.startDate FROM Appointment AS appointment WHERE appointment.voided = 0)";
...
/*            if (visitType != null)
            ...
            query.setParameter("fromDate", getStringDate(fromDate));
            query.setParameter("toDate", getStringDate(toDate));
...
/*        if (visitType != null)
            ...
            log.error("missed appnts  " + query.list().size());
    ...
    @Override
    public List<Appointment> getDefaultersList(int minDays, int maxDays, Provider provider, AppointmentType appointmentType, VisitType visitType, Location location) throws DAOException {
        String stringQuery = null;
